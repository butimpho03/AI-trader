"""
TradeLens AI — Phase 10 (single-file edition)
...
more evidence exists). Phase 10 (this version) adds the level
calculation engine: real hypothetical entry/stop/TP1/TP2 numbers,
finally. Strictly split in two, per the project's core rule ("AI
explains, engine calculates") — the AI only reads real numbers off the
LTF chart's visible price axis (current price, nearest swing high/low),
and 100% deterministic Python code does all the entry/stop/target math
from there, with a fixed 1.5R/3R target ladder. It refuses to produce a
setup (returns NO VALID SETUP with reasons) whenever the candidate
direction is unclear, a timeframe actively conflicts, the needed price
points can't be read, or the resulting risk/reward doesn't clear a
1:1.2 floor — verified against the project's own conflict example
before shipping. Named technical strategies (BOS, FVG, order blocks,
RSI, etc.), backtesting, paper trading, and the journal are still not
...
class PriceLevelReading(BaseModel):
    """Phase 10: real numbers read directly off a chart's price axis/
    labels — NOT calculated, NOT estimated, NOT invented. This is OCR-
    style reading of what's visibly printed on the LTF chart (the same
    thing that already correctly read "4,626.975" in Phase 6 testing).
    Any field the AI isn't confident about must come back None rather
    than a guess — the level-calculation engine below refuses to
    proceed on missing data rather than substituting a made-up number."""
...
class GeminiAIProvider(AIProvider):
    ...
    def _call_gemini_vision(self, prompt: str, image_bytes: bytes) -> Tuple[Optional[str], Optional[str]]:
        """Shared request/error-handling for any single-image Gemini call.
        ...
        if resp.status_code in (500, 502, 503, 504):
            return None, (
                ...
                "This is on Google's side, not a bug here — wait 30-60 seconds and try again."
            )
...
def footer() -> None:
    st.markdown(
        '<div class="tl-footer">TradeLens AI · Phase 10 · Educational tool, not financial advice</div>',
        unsafe_allow_html=True,
    )
...
def page_dashboard() -> None:
    ...
    card(
        "Phase 10 status",
        "Real hypothetical entry/stop/TP numbers now exist — see \"Hypothetical Setup\" "
        "on Active Setups once a session has all 3 charts read. Numbers come from real "
        "price labels the AI reads off the chart, with 100% deterministic math on top — "
        "never guessed. Often correctly returns NO VALID SETUP when evidence is thin.",
        icon="🧭",
    )
    card(
        "Important",
