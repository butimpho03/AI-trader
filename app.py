"""
TradeLens AI — Phase 7 (single-file edition)
...
(Gemini free tier) reading one chart at a time. Phase 7 (this version)
adds a real multi-timeframe comparison: once all three charts are
attached, the AI reads each one's visible price-shape impression
(bullish/bearish/neutral/unclear) and the app compares HTF vs MTF vs
LTF, flagging agreement or conflict exactly like the project brief's
example ("HTF/MTF bullish, but LTF bearish — not confirmed"). This is
still NOT a trading signal, evidence, or a quality score — those need
real strategy modules and a confluence engine, which are still not
...
def footer() -> None:
    st.markdown(
        '<div class="tl-footer">TradeLens AI · Phase 7 · Educational tool, not financial advice</div>',
        unsafe_allow_html=True,
    )
...
def page_dashboard() -> None:
    ...
    card(
        "Phase 7 status",
        "Multi-timeframe comparison is now real — once all 3 charts are "
        "attached, run \"Compare timeframes\" on Active Setups to see HTF/MTF/LTF "
        "visual impressions and where they agree or conflict. Still not a trading "
        "signal or quality score — the strategy engine is still not implemented.",
        icon="🧭",
    )
    card(
        "Important",
