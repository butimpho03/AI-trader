"""
TradeLens AI — Phase 3 (single-file edition)
...
added real instrument resolution (aliases, normalization, manual
override). Phase 3 (this version) adds real session management: you can
now create an AnalysisSession per instrument and see it listed on
Active Setups. Sessions live in Streamlit's session_state (this browser
tab only) — there is no database yet, so they don't survive a refresh
or app restart. Chart upload, AI vision, and the strategy engine are
...
def footer() -> None:
    st.markdown(
        '<div class="tl-footer">TradeLens AI · Phase 3 · Educational tool, not financial advice</div>',
        unsafe_allow_html=True,
    )
...
def page_dashboard() -> None:
    ...
    card(
        "Phase 3 status",
        "Sessions are now real (for this browser tab) — instrument search, "
        "resolution, and session creation all work. Chart upload, AI vision, "
        "and the strategy engine are still not implemented.",
        icon="🧭",
    )
    card(
        "Important",
...
def page_active_setups() -> None:
    ...
    st.caption(
        ...
        "instrument — its charts (once upload exists) can never mix with another session's."
    )
    status_tone = {
    ...
    for s in sessions:
        chart_count = sum(x is not None for x in [s.htf_chart_id, s.mtf_chart_id, s.ltf_chart_id])
        card(
