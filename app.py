"""
TradeLens AI — Phase 4 (single-file edition)
...
management. Phase 4 (this version) adds real three-chart upload: from
Active Setups, each session now has HTF / MTF / LTF upload slots, and a
session's status becomes READY once all three are attached. Uploaded
images live in Streamlit's session_state (this browser tab only) — not
saved to disk or a database yet. Chart quality checking, AI vision, and
the strategy engine are still not implemented — every placeholder below
...
class ChartUpload(BaseModel):
    ...
    quality: ChartQuality = ChartQuality.UNKNOWN  # real quality checking is a later phase
...
def attach_chart(session: AnalysisSession, role: TimeframeRole, uploaded_file) -> ChartUpload:
    """Creates a ChartUpload for the given role, stores its image bytes,
    attaches it to the session, and updates the session's status. This
    is the only function that may set a session's htf/mtf/ltf_chart_id —
    keeping that in one place is what prevents a chart from ever being
    attached to the wrong session by accident."""
    ...
    _chart_images()[chart_id] = uploaded_file.getvalue()
...
def footer() -> None:
    st.markdown(
        '<div class="tl-footer">TradeLens AI · Phase 4 · Educational tool, not financial advice</div>',
        unsafe_allow_html=True,
    )
...
def page_dashboard() -> None:
    ...
    card(
        "Phase 4 status",
        "Sessions and chart uploads are now real (for this browser tab) — "
        "attach HTF/MTF/LTF images from Active Setups and status updates to "
        "READY automatically. Chart quality checking, AI vision, and the "
        "strategy engine are still not implemented.",
        icon="🧭",
    )
    card(
        "Important",
