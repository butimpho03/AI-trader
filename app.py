"""
TradeLens AI — Phase 8 (single-file edition)
...
(this version) adds a real strategy engine: the StrategyModule/registry
architecture (scaffolded since Phase 0, empty until now) runs for the
first time, with exactly ONE honest module — "AI Visual Trend Reading"
— that turns Phase 7's cached per-timeframe reads into formal
StrategyResult evidence, organized by evidence layer (market context /
location / confirmation). Named technical strategies (BOS, CHoCH, FVG,
order blocks, RSI, etc.) still aren't implemented, because they need
real structured price/indicator data this app doesn't have — adding
them now would mean inventing structure that isn't verifiably there.
No quality score exists yet either — that's Phase 9 (confluence
engine). Every placeholder below says so honestly instead of pretending
to work.
    ...
    for role, chart_id in chart_ids.items():
        ...
        if cached is None:
            ...
            results.append(StrategyResult(
                ...
                quality_contribution=0.0,  # weighting requires the confluence engine — Phase 9
                insufficient_data=False,
                notes=["Not yet weighted into a quality score — that's Phase 9 (confluence engine)."],
            ))
...
def footer() -> None:
    st.markdown(
        '<div class="tl-footer">TradeLens AI · Phase 8 · Educational tool, not financial advice</div>',
        unsafe_allow_html=True,
    )
...
def page_dashboard() -> None:
    ...
    card(
        "Phase 8 status",
        "The strategy engine now actually runs — see \"Strategy Engine\" on Active "
        "Setups (below Compare Timeframes) once a session has all 3 charts read. "
        "One honest module so far, organized by evidence layer. No quality score "
        "yet — that's Phase 9 (confluence engine).",
        icon="🧭",
    )
    card(
        "Important",
                    ...
                    for r in layer_results:
                        if r.insufficient_data:
                            ...
                            for ev in r.evidence:
                                st.caption(f"↳ {ev}")
                st.caption(
                    "No quality score yet — combining this evidence into one transparent "
                    "setup-quality number is Phase 9 (confluence engine)."
                )
