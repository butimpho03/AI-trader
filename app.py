"""
TradeLens AI — Phase 5 (single-file edition)
...
management. Phase 4 added real three-chart upload. Phase 5 (this
version) adds real chart quality checking: every uploaded chart is
automatically scored GOOD/FAIR/POOR based on resolution, aspect ratio,
and a blur estimate, plus duplicate-image detection within a session.
This is deterministic, offline image processing — NOT AI vision. It
cannot tell you what instrument or timeframe is shown, or whether the
candles are actually readable to a human; only whether the file is
technically usable. Real AI vision is Phase 6. The strategy engine is
still not implemented — every placeholder below says so honestly
...
def footer() -> None:
    st.markdown(
        '<div class="tl-footer">TradeLens AI · Phase 5 · Educational tool, not financial advice</div>',
        unsafe_allow_html=True,
    )
...
def page_dashboard() -> None:
    ...
    card(
        "Phase 5 status",
        "Chart quality checking is now real — resolution, cropping, blur "
        "estimate, and duplicate detection all run automatically on upload. "
        "This is image analysis, not AI vision — it can't read chart content "
        "yet. AI vision and the strategy engine are still not implemented.",
        icon="🧭",
    )
    card(
        "Important",
